...
var rockPaperScissors = function() {
  ...
  var renderScore = function() {
    //reusable function to render/reset score
  ...
  var displayMessage = function(message, messageDuration) {
    if ($("#message").length) {
      ...
      $jsBattleDome.append(
        '<p id="message" aria-hidden="false" role="alert" class="message-text">' +
          message +
          "</p>"
      );
    }
  ...
  var renderGamePlay = function(userChoice, randomChoice) {
    ...
    setTimeout(function() {
      $("#left-hand")
        .removeClass()
        .addClass(userChoice)
        .text("player one plays: " + userChoice);
      $("#right-hand")
        .removeClass()
        .addClass(randomChoice)
        .text("player two plays: " + randomChoice);
      renderScore();
      if (gameMode != 0) {
        checkStatus();
      }
  ...
  var activateSettingPanel = function() {
    ...
    if ($("#settings-pane").hasClass("active")) {
      ...
      $("#message, #js-reset, .actions")
        .addClass("hidden")
        .attr("aria-hidden", "true");
    } else {
      $("#js-settings, #js-reset, .actions")
        .attr("aria-hidden", "true")
        .removeClass("hidden");
